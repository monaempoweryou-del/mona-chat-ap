...
| 2026-06-05 | VALIDATE-001 (confirmed): Never automate before real-world testing. The Renova Visual Training workflow was initially built as an image review tool — testing revealed it was actually a visual training framework. The BIE was initially designed with a Step 2 gate — testing revealed the full report can be delivered without Step 2 answers. In both cases, the test corrected a wrong assumption before automation would have locked it in. |
| 2026-06-05 | Gmail MCP (d1d24a0c) is read/draft only — no send capability. Drafts can be created autonomously; sending to external recipients requires Maor to open Gmail and click Send. This is the minimum irreducible human touchpoint for all client-facing email workflows. Do not design workflows that assume autonomous outbound email. |
| 2026-06-05 | BIE workflow can execute Steps 1–9 autonomously. The MONA Business Intelligence Engine — from reading the lead email to generating the full HTML report to creating 3 follow-up drafts — runs without requiring Maor input. Only genuine blockers: (1) Gmail send (1 click), (2) website content behind WAF (requires browser/Playwright), (3) social media content (requires browser). |
| 2026-06-05 | NAP audit: always search "[Business Name] + [all known previous cities]" explicitly. A business that moved cities will have stale listings in old-city directories that don't appear in current-city searches. For Goldsmith Financial, searching "Goldsmith Financial San Diego" surfaced outdated Yelp/Yahoo Local listings that were invisible in Temecula searches. |
| 2026-06-05 | Competitive landscape research requires 3+ search rounds with different keyword combinations. First search returns 4–5 top competitors. Additional searches (e.g., "retirement planning [city]", "[service] advisor [city] 2026", "[service] near me [city]") reveal 3–4 more. Never declare the competitive set complete after one search. |
| 2026-06-05 | LPL Financial advisor testimonials: the 2022 SEC Marketing Rule ended the blanket prohibition on client testimonials for investment advisers. LPL-affiliated advisors can now use testimonials in marketing via LPL ClientWorks MRR review process. Required disclosures: (1) current client status, (2) no compensation, (3) no material conflicts. Organic Google reviews do not require pre-approval. Most advisors are still operating under the old (prohibited) assumption — this is a major untapped opportunity. |
| 2026-06-05 | BIE report delivery does not require Step 2 answers. Deliver the full report with an "Open Intelligence Gaps" section noting which findings will be refined after Step 2 responses arrive. This reduces time-to-value and makes Step 2 a refinement, not a gate. |
| 2026-06-05 | The real success condition of a workflow is not "completed." It is "consistently produces an approved result." A workflow that ran once and produced output is not validated. Validated means: the workflow ran multiple times and the output was approved by all relevant parties (Maor + client). |
